...
※ `coder`，`tester`，`refactorer` は `.claude/agents/` に定義されたエージェントであり，`/implement` のオーケストレーターが Agent ツール経由で呼び出す．個別実行する場合も，各エージェントを Agent ツールで順に呼び出す．

```
# 個別実行の流れ
Agent(coder)       # Phase 1
# → 人間が動作確認
Agent(tester)      # Phase 2
# → 人間がテスト結果確認
Agent(refactorer)  # Phase 3
# → 人間が最終確認 → コミット
```

## コミットルール (Commit Rules)

パイプライン完了後は `/commit` でコミットする．コミットの詳細は [GUIDE_04](GUIDE_04_Git運用ルール.md) に従う．
