...
2. **変更後**，`.claude/template-overrides.md` の「台帳」表に 1 行追加する（既に行があるファイルは理由を最新化する）．書式・方針（`keep` / `merge` / `ask`）の定義は同ファイルの「記録ルール」節に従う
   - 方針は，テンプレート側の改善も今後取り込みたいなら `merge`（既定．改変箇所とテンプレートの差分が別の箇所なら自動で両立する），完全に別物にしたなら `keep`，都度判断したいなら `ask`
...
- 台帳の書式を崩さない（`/sync-template` が機械的に読む）
