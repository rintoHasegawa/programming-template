...
1. **コーディング規約を読む**: プロジェクトのコーディング規約（`.claude/rules/` の coding-style 等，存在する場合）を確認する
2. **サマリーを確認する**: Coder・Tester エージェントの出力を読む
3. **変更差分を確認する**: `git diff` で全変更内容を確認する
4. **テストを実行する**: 現時点でテストが全て通ることを確認する（開始前のベースライン）．テスト実行コマンドは `docs/02_ENV/ENV_04_開発コマンド.md`（存在する場合）に記載のものを使い，無ければ構成ファイルから特定する
